...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.token_type_embeddings_new = nn.Embedding(4, config.hidden_size, padding_idx=0)
    ...
    def forward(self, input_ids, token_type_ids=None):
        ...
        token_type_embeddings = self.token_type_embeddings_new(token_type_ids)
...
class VisualFeatEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.vis_box_fc = nn.Linear(config.hidden_size * 2, config.hidden_size)
        self.vis_box_layer_norm = BertLayerNorm(config.hidden_size, eps=1e-12)
    ...
    def forward(self, visn_input):
        ...
        output = self.vis_box_fc(torch.cat([x, y], dim=-1))
        output = self.vis_box_layer_norm(output)
...
class BertPooler_score(nn.Module):
    def __init__(self, config):
        super(BertPooler_score, self).__init__()
        self.dense = nn.Linear(config.hidden_size, 1)

    def forward(self, lang_feat, vis_feat):
        # We "pool" the model by simply taking the hidden state corresponding
        # to the first token.
        first_lang_feat = lang_feat[:, 0]
        first_vis_feat = vis_feat[:, 0]

        pooled_output = self.dense(first_vis_feat * first_lang_feat)
...
class LXRTModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.pooler_score = BertPooler_score(config)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        pooled_output = self.pooler_score(lang_feats, visn_feats)
...
class LXRTPretraining(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
                visual_feats=None, visual_feats_mask=None, pos=None, obj_labels=None, matched_label=None, ans=None):
        (lang_output, visn_output), pooled_output = self.bert(
            input_ids, token_type_ids, attention_mask,
            visual_feats=(visual_feats, pos), visual_attention_mask=visual_feats_mask
        )

        bsz = ans.size(0)

        answer_score = pooled_output.view(bsz, 4)

        return answer_score, lang_output, visn_output
